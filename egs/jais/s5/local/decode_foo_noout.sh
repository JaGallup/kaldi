...
#data=$2
#dir=$3
modeldir=expold/tri3a
graphdir=$modeldir/graph_gallup_min
...
steps/make_mfcc.sh --nj 1 --cmd run.pl --mfcc-config conf/mfcc.conf foo foo/make_mfcc foo/mfcc &> /tmp/mfcc
steps/compute_cmvn_stats.sh foo foo/make_mfcc foo/mfcc &> /tmp/cmvn
...
if [ $stage -le 0 ]; then
  ...
  gmm-latgen-faster --max-active=$max_active --beam=$beam --lattice-beam=$lattice_beam \
    ...
    $model $graphdir/HCLG.fst "$feats" "ark:|gzip -c > $dir/lat.1.gz" &> /tmp/gmm || exit 1;
  touch /tmp/after
fi

lattice-best-path --word-symbol-table=$graphdir/words.txt "ark:gunzip -c $modeldir/decode_foo/lat.1.gz|" ark,t:$modeldir/decode_foo/foo.tra &> /tmp/latt
