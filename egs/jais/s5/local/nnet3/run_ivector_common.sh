...
if $speed_perturb; then
  ...
  if [ $stage -le 2 ] && $generate_alignments; then
    ...
    steps/align_fmllr.sh --nj 100 --cmd "$train_cmd" \
      data/${train_set}_sp data/lang exp/tri4 exp/tri4_ali_sp
  fi
...
if [ $stage -le 3 ]; then
  ...
  for dataset in eval2000 test train_dev; do
  ...
  utils/data/remove_dup_utts.sh 200 data/${train_set}_30k_hires data/${train_set}_30k_hires  # 33hr
fi
...
if [ $stage -le 6 ]; then
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 30 --num-frames 200000 \
    data/${train_set}_30k_hires 512 exp/nnet3/tri3b exp/nnet3/diag_ubm
fi
